...
#
# Interface:
#           SSIM, SRE, RMSE, ERGAS, SAM = EvaluatePerformance(Xm_im,...
#                                                             Xhat_im,...
#                                                             limsub,...
#                                                             data_range,...
#                                                             d, bands)
...
def EvaluatePerformance(Xm_im, Xhat_im, limsub=0, data_range=1.0, d=[6, 1, 1, 1, 2, 2, 2, 1, 2, 6, 2, 2], bands=[2,6]):
    if isinstance(Xhat_im, np.ndarray):
        average = False
    elif isinstance(Xhat_im, list):
        average = True
    else:
        raise TypeError(f"Expected numpy array or list but got {type(Xhat_im)}")
    ...
    if average:
        ...
        for k in range(len(Xhat_im)):
            res = EvaluatePerformance(Xm_im, Xhat_im[k], limsub=limsub, data_range=data_range, bands=bands)
...
def sreval(Xm_im, Xhat_ims, limsub=0, data_range=1.0,
    ...
    if not isinstance(Xhat_ims, list):
        Xhat_ims = [Xhat_ims]
    res_list = [{} for i in range(len(Xhat_ims))]
    for res_dict, Xhat in zip(res_list, Xhat_ims):
        res_dict['SSIM'], res_dict['SRE'], res_dict['RMSE'], res_dict['ERGAS'], res_dict['SAM'], res_dict['UIQI'] = EvaluatePerformance(Xm_im, Xhat, limsub=limsub, data_range=data_range, d=d, bands=bands)
    return res_list


# Main functions fusion
...
#
# Interface:
#           SSIM, SRE, RMSE, ERGAS, SAM = EvaluateFusionPerformance(Xm_im,...
#                                                                   Xhat_im,...
#                                                                   limsub,...
#                                                                   data_range,...
#                                                                   d, bands)
...
def EvaluateFusionPerformance(Xm_im, Xhat_im, limsub=0, data_range=1.0,
                         d = None,
                         bands=[2, 3, 6]):
    if d is None:
    ...
    if average:
        SSIM = {"20m": [], "30m": [], "60m": [], "All": [], "Bands": []}
        SRE = {"20m": [], "30m": [], "60m": [], "All": [], "Bands": []}
        UIQI = {"20m": [], "30m": [], "60m": [], "All": [], "Bands": []}
        RMSE = {"20m": [], "30m": [], "60m": [], "All": []}
        ERGAS = {"20m": [], "30m": [], "60m": []}
        SAM = {"20m": [], "30m": [], "All": []}
        metrics = [SSIM, SRE, RMSE, ERGAS, SAM, UIQI]

        for k in range(len(Xhat_im)):
            res = EvaluateFusionPerformance(Xm_im, Xhat_im[k], limsub=limsub, data_range=data_range, bands=bands)
            for metric, result in zip(metrics, res):
                for key, value in result.items():
                    metric[key].append(value)
        for metric in metrics:
        ...
        if 3 in bands:
            idx3 = np.squeeze(np.where(d == 3))
            ERGAS["30m"] = ergas(X[:,:,idx3], Xhat[:,:,idx3], 3)
            RMSE["30m"] = rmse(X[:,:,idx3], Xhat[:,:,idx3])
            SAM["30m"] = sam(X[:,:,idx3], Xhat[:,:,idx3])
            SSIM["30m"] = SSIM_array[idx3].mean()
            SRE["30m"] = SRE_array[idx3].mean()
            UIQI["30m"] = UIQI_array[idx3].mean()
        ...
        if 6 in bands and 3 in bands and 2 in bands:
            RMSE["All"] = rmse(X[:,:,idx], Xhat[:,:,idx])
            SAM["All"] = sam(X[:,:,idx], Xhat[:,:,idx])
            SRE["All"] = SRE_array[idx].mean()
            UIQI["All"] = UIQI_array[idx].mean()
...
#
# Interface:
#           res_list = fsreval(Xm_im, Xhat_ims, limsub, data_range, d, bands)
#
# Inputs:
#           Xm_im:          Ground truth image;
#           Xhat_ims:       List of (lists of) estimated images;
...
def fsreval(Xm_im, Xhat_ims, limsub=0, data_range=1.0,
           d=[6, 1, 1, 1, 2, 2, 2, 1, 2, 6, 2, 2, 3, 3, 3, 3, 3, 3, 3, 1, 2], bands=[2,3,6]):
    if not isinstance(Xhat_ims, list):
        Xhat_ims = [Xhat_ims]
    res_list = [{} for i in range(len(Xhat_ims))]
    for res_dict, Xhat in zip(res_list, Xhat_ims):
        res_dict["SSIM"], res_dict["SRE"], res_dict["RMSE"], res_dict["ERGAS"], res_dict["SAM"], res_dict["UIQI"] = EvaluateFusionPerformance(Xm_im, Xhat, limsub=limsub, data_range=data_range, d=d, bands=bands)
    return res_list
